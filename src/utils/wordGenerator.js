...
export const generateLegislativeProject = async (report) => {
  ...
  const locationFormatted = `${report.calle_principal || ''} ${report.entre_calle_1 && report.entre_calle_2 ? `(entre ${report.entre_calle_1} y ${report.entre_calle_2})` : ''}`.trim();
  let location = report.location || locationFormatted || 'la ciudad';

  const locLower = location.toLowerCase();
  if (locLower.includes('sin ubicación') || locLower.includes('ubicación no encontrada') || locLower === '') {
    location = 'el área de influencia referenciada en los reportes vecinales';
  }

  const catLower = report.category.toLowerCase();
  
  if (catLower.includes('calles') || catLower.includes('asfalto') || catLower.includes('baches')) {
    actionText = `proceda a realizar de manera urgente las obras de bacheo, reparación y reacondicionamiento asfáltico en la vía pública ubicada en ${location}`;
    ...
    actionText = `proceda a la urgente reparación, mantenimiento y/o recambio de las luminarias pertenecientes al sistema de alumbrado público en la zona de ${location}`;
    ...
    actionText = `proceda a llevar a cabo tareas intensivas de desmalezamiento, limpieza y mantenimiento integral de los espacios públicos y/o baldíos ubicados en ${location}`;
    ...
    actionText = `proceda a ejecutar tareas de limpieza profunda y la consecuente erradicación de minibasurales o focos de contaminación en la vía pública ubicada en ${location}`;
    ...
    actionText = `proceda a la urgente reparación, mantenimiento y/o instalación de dispositivos de ordenamiento del tránsito y señalización vial correspondiente en la intersección/zona de ${location}`;
    ...
    actionText = `interceda y articule acciones ante el Ministerio de Gobierno y la Policía de la Provincia a los fines de solicitar el inmediato refuerzo de patrullajes preventivos y mayor presencia policial en la zona de ${location}`;
    ...
    actionText = `proceda a comisionar de forma urgente una cuadrilla técnica para realizar las tareas de remoción, reparación o aseguramiento de estructuras o especies arbóreas con riesgo inminente de caída en la vía pública ubicada en ${location}`;
    ...
    // Genérico / Otros
    const isOtros = report.category && report.category.toLowerCase() === 'otros';
    const problem = isOtros ? 'las diversas problemáticas expuestas por los vecinos' : `la problemática vinculada a ${report.category}`;
    actionText = `proceda a intervenir de forma prioritaria para dar solución a ${problem} en ${location}`;
